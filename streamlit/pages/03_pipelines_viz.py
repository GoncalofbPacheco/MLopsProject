...
def launch_kedro_viz_server(reporter):
    if not st.session_state["kedro_viz_started"]:
        ...
        good_process = subprocess.run(
            ["kedro", "viz", "--no-browser"], cwd="../", capture_output=True, text=True
        )
...
def show_pipeline_viz():
    ...
    st.markdown(
        "<small>This page shows an interactive visualization of the Kedro pipelines in the project to help understand data flow and dependencies.</small>",
