...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_clean = df_clean[
        (df_clean["age"] >= 0)
        & (df_clean["age"] <= 90)
        & (df_clean["resting_bp_s"] >= 60)
        & (df_clean["resting_bp_s"] <= 200)
        & (df_clean["cholesterol"] >= 100)
        & (df_clean["cholesterol"] <= 500)
        & (df_clean["max_heart_rate"] >= 60)
        & (df_clean["max_heart_rate"] <= 220)
    ]

    after_rows = df_clean.shape[0]
    logger.info(
        f"Cleaned data: removed {before_rows - after_rows} rows due to outliers."
    )
...
def feature_engineer(data: pd.DataFrame):
    ...
    encoded_cat = pd.DataFrame(
        encoder.fit_transform(df[categorical_cols]), index=df.index
    )
