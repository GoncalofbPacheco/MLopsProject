...
def model_train(
    ...
    with open("conf/local/mlflow.yml") as f:
        experiment_name = yaml.load(f, Loader=yaml.loader.SafeLoader)["tracking"][
            "experiment"
        ]["name"]
    experiment_id = mlflow.get_experiment_by_name(experiment_name).experiment_id
    logger.info(
        "Starting first step of model selection : Comparing between modes types"
    )
    ...
    try:
        with open(
            os.path.join(os.getcwd(), "data", "06_models", "champion_model.pkl"), "rb"
        ) as f:
    ...
    shap.summary_plot(
        shap_values[:, :, 1], X_train, feature_names=X_train.columns, show=False
    )
