...
def pdays_(data):
    ...
    data.loc[
        (data["months_passed"] >= 0) & (data["months_passed"] <= 2), "months_passed"
    ] = 1
    data.loc[
        (data["months_passed"] > 2) & (data["months_passed"] <= 6), "months_passed"
    ] = 2
...
def feature_engineer(data: pd.DataFrame, OH_encoder) -> pd.DataFrame:
    ...
    df["z_score_bin_age"] = (df["mean_balance_bin_age"] - df["balance"]) / (
        df["std_balance_bin_age"]
    )
